...
def addParentId(df, account_id):
    df["adgroup_id"] = df.apply(lambda row: functions.addIdToDf(account_id, row["google_adgroup_id"], row["campaign_google_id"]), axis=1)
    return df
...
def reportSpecificProcessing(df, date_range, account_id):
    ...
    df["advert_id"] = df.apply(lambda row: functions.addIdToDf(account_id, row["google_id"], row["google_adgroup_id"]), axis=1)
